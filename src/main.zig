...
fn printProfit(
    ...
    const start_date_pretty = start_date.prettyPrint(allocator);
    const end_date_prettry = end_date.prettyPrint(allocator);
    defer allocator.free(end_sum);
    defer allocator.free(start_sum);
    defer allocator.free(delta);
    defer allocator.free(start_date_pretty);
    defer allocator.free(end_date_prettry);
    try writer_stdout.writeAll("\n##########################################################\n\n");
    try writer_stdout.print("Дата открытия депозита:                {s}\n", .{start_date_pretty});
    try writer_stdout.print("Дата закрытия депозита:                {s}\n", .{end_date_prettry});
    try writer_stdout.print("Срок депозита в месяцах:               {d}\n\n", .{profit_info.months});
